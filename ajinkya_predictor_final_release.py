"""
Ajinkya's Predictor - Final Release
- UI title: Ajinkya's Predictor (appealing header)
- Keeps RandomForest + Ridge; removes LightGBM.
- Adds final risk control: Historical VaR, Parametric VaR (Gaussian), and CVaR (Expected Shortfall).
- UI outputs only requested fields: CurrentPrice, PredictedSharePrice, Momentum, Fundamentals Score,
  Implied Return, Practical Range, Company Description, Sentiment Description, Confidence Level, Trend,
  Descriptive Recommendation with beginner-friendly buy/sell/hold guidance and price levels.
- Designed to be deploy-friendly and stable.
"""

import streamlit as st
import pandas as pd
import numpy as np
import requests, math, warnings, traceback
...
def fetch_prices_yf(ticker: str, days=1500):
    ...
    try:
        t = yf.Ticker(ticker)
        end = datetime.now(); start = end - timedelta(days=days*1.1)
        df = t.history(start=start.strftime("%Y-%m-%d"), end=end.strftime("%Y-%m-%d"), interval="1d", actions=False)
        if df.empty: return None
        df = df.reset_index()[['Date','Close','High','Low','Volume']].rename(columns={'Date':'date','Close':'close','High':'high','Low':'low','Volume':'volume'})
        df['date'] = pd.to_datetime(df['date']); df = df.sort_values('date').reset_index(drop=True)
        return df
    except Exception:
        return None

# --- core utilities & indicators (kept concise) ---
...
def rsi(series, period=14):
    delta = series.diff()
    up = delta.clip(lower=0).fillna(0); down = -1*delta.clip(upper=0).fillna(0)
...
def kalman_smooth(series):
    ...
    for k in range(1, n):
        ...
        K = Pminus / (Pminus + R); xhat[k] = xhatminus + K * (series.iloc[k] - xhatminus); P[k] = (1 - K) * Pminus
...
def ridge_predict(df, days_ahead=3):
    ...
    if len(df2) < 40 or len(cols)==0: return float(df['close'].iloc[-1])
    X = df2[cols]; y = df2['close']; split = max(5, int(0.8*len(X)))
    try:
        model = Ridge(alpha=1.0); model.fit(X.iloc[:split], y.iloc[:split]); return float(model.predict(X.iloc[-1:].values.reshape(1,-1))[0])
    except Exception:
        return float(df['close'].iloc[-1])
...
def rf_predict(df, days_ahead=3):
    ...
    if len(df2) < 80: return float(df['close'].iloc[-1])
    X = df2[cols]; y = df2['close']; split = max(10, int(0.8*len(X)))
    model = RandomForestRegressor(n_estimators=80, max_depth=7, random_state=1, n_jobs=1)
    model.fit(X.iloc[:split], y.iloc[:split]); return float(model.predict(X.iloc[-1:].values.reshape(1,-1))[0])
...
def ensemble_predict(df, days_ahead=3):
    p_hw = hw_predict(df, days_ahead); p_ridge = ridge_predict(df, days_ahead); p_rf = rf_predict(df, days_ahead)
    p_ar1 = ar1_predict(df, days_ahead); p_spec = fft_dominant_forecast(df['close'], days_ahead=days_ahead)
    ...
    if hurst > 0.55:
        weights["trend"] += 0.10; weights["spec"] -= 0.05; weights["ar1"] -= 0.05
    if vol > 0.06:
        weights["spec"] += 0.10; weights["rf"] += 0.05; weights["trend"] -= 0.05
    arr = np.array(list(weights.values())); arr = np.clip(arr, 0.01, 0.9); arr = arr / arr.sum()
    wkeys = list(weights.keys()); weights = dict(zip(wkeys, [float(x) for x in arr]))
...
# Risk controls: VaR & CVaR (Expected Shortfall)
def historical_var(returns, alpha=0.05):
    # returns is a series of daily returns (not log)
    if len(returns) < 10:
        return 0.0
    sorted_r = np.sort(returns)
    idx = max(0, int(alpha * len(sorted_r)) - 1)
    return -sorted_r[idx]  # positive number

def parametric_var(returns, alpha=0.05):
    # Gaussian VaR: mu + sigma * z_alpha (z negative), return positive magnitude
    if len(returns) < 10:
        return 0.0
    mu = np.mean(returns); sigma = np.std(returns)
    z = -np.abs(np.percentile(np.random.normal(size=100000), alpha*100))  # approximate z
    var = -(mu + sigma * z)
...
# scoring & recommendation (beginner-friendly)
...
def compute_fundamentals_score(profile_fmp, profile_yf):
    ...
    try:
        if profile_fmp and isinstance(profile_fmp, dict):
            mcap = float(profile_fmp.get('mktCap',0) or 0); pe = float(profile_fmp.get('pe',0) or 0)
            roe = float(profile_fmp.get('returnOnEquity',0) or 0)
            if mcap>0: score += 5
            if pe>0 and pe<30: score += 5
            if roe>0.05: score += 5
    except Exception:
        pass
    try:
        if profile_yf and isinstance(profile_yf, dict):
            ...
            if pe and pe>0 and pe<30: score += 5
            roe = profile_yf.get('returnOnEquity') or profile_yf.get('roe')
            if roe and roe>0.05: score += 5
    except Exception:
        pass
...
# sentiment (simple) and description
...
def simple_sentiment_desc(news_items):
    # returns (score between -1..1, short human readable sentence)
    score = 0.0; count = 0
    for n in news_items[:60]:
        txt = " ".join([str(n.get(k,"")) for k in ("title","text")]).lower(); s=0
        for w in POS:
            if w in txt: s+=1
        for w in NEG:
            if w in txt: s-=1
...
def recommendation_text(pred, cur, lower, upper, implied_return, conf, sentiment_score, sentiment_desc, momentum_score, fundamentals_score, trend):
    ...
    lines.append("")
    # price levels to guide beginners
    buy_price = cur * (1 - 0.03)  # suggested buy ~3% below current as an example
    strong_buy_price = cur * (1 - 0.08)
    take_profit = cur * (1 + max(0.06, change/2/100))  # at least 6% or half predicted change
...
if submitted:
    ...
    if hist is None or len(hist) < 80:
        ...
        # PCA compress if available (non-mandatory)
        if HAVE_SK:
            try:
        ...
        profile_yf = None; desc = fetch_company_description(fmp_key, ticker)
        if HAVE_YFINANCE:
            try:
        ...
        conf = 0.5 + 0.28*(1 - min(1, vol*10)); conf = max(0.12, min(0.98, conf))
        ...
        # risk calculations on daily returns
        ...
        # Display UI: only specified fields, attractive formatting
